...
savefolder = pathlib.Path("/store/DAMTP/smrsi2/trained_models/test_debbuging/") # Does it exist?
...
# %% Dataset
lidc_dataset = experiment.get_training_dataset()                                                    # Need to ask!
lidc_dataset_val = experiment.get_validation_dataset()                                              # Need to ask!
lidc_dataset_test = experiment.get_testing_dataset()                                                # Need to ask!
...
batch_size = 1
lidc_dataloader = DataLoader(lidc_dataset, batch_size, shuffle=True)                                # Need to ask!
lidc_validation = DataLoader(lidc_dataset_val, batch_size, shuffle=False)                           # Need to ask!
lidc_test = DataLoader(lidc_dataset_test, batch_size, shuffle=False)                                # Need to ask!
...
model = FBPConvNet(experiment.geometry, default_parameters)
model.cite()
model.cite("bib")
