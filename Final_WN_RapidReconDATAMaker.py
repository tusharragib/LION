...
# Set target CUDA device before importing PyTorch/TIGRE dependencies
...
def pre_flip(projection: np.ndarray) -> np.ndarray:
    """Flips projection data along the first two spatial axes (height and width)."""
...
def log_transform(projection: np.ndarray) -> np.ndarray:
    ...
    Transforms values into line integrals of linear attenuation coefficients.
    Formula: -log((I + 1) / (I_max + 1))
    """
    projection = projection.astype(np.float32)
    
    # Compute max intensity per projection frame along axis 0
    max_intensities = projection.max(axis=(1, 2), keepdims=True)
    
    # Vectorized log transformation across all frames
...
def apply_circular_mask(reconstruction: np.ndarray, radius: int = 175) -> np.ndarray:
    ...
    # Define circular center off-set relative to grid midpoint
    cy, cx = H // 2 + 5, W // 2 - 5
    ...
    # Broadcast mask across 3D slice dimension
...
def main():
    ...
    output_dir = Path("/store/cia-lion/smrsi2/WNHospitalData/3D_recon/data_LOW")
    ...
    # Iterative OS-ASD-POCS Reconstruction Settings
    blocksize = 17
    num_iter = 15
    alpha = 0.0001

    for fp in tqdm(filtered_files, desc="Processing CT Volumes"):
        try:
            ...
            # 3. Calculate rotation angles in radians
            ...
            # 4. Update dynamic detector parameters
            update_detector_geometry(geo_TIGRE, len(angles))

            # 5. Perform OS-ASD-POCS Reconstruction
            ...
            # 7. Save reconstructed array
            suffix_parts = fp.stem.split("_")[2:]
            new_filename = f"ReconVolume_3D_{'_'.join(suffix_parts)}_data.npy"
